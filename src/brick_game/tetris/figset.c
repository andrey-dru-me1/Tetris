...
static figure_t figgen_generate_I_figure() {
  ...
  for (size_t i = 0; i < 4; i++) bitmatrix_set_bit(&bm1, i, 2, 1);
...
static figure_t figgen_generate_L_figure() {
  ...
  for (size_t i = 0; i < 3; i++) bitmatrix_set_bit(bms + 0, i, 1, 1);
  bitmatrix_set_bit(bms + 0, 2, 2, 1);
...
static figure_t figgen_generate_T_figure() {
  ...
  for (size_t i = 0; i < 3; i++) {
    bitmatrix_set_bit(bms + 0, 1, i, 1);
  }
  bitmatrix_set_bit(bms + 0, 0, 1, 1);
...
static figure_t figgen_generate_S_figure() {
  ...
  bitmatrix_t bms[2];
  bitmatrix_create(bms + 0, 3, 3);
  bitmatrix_set_bit(bms + 0, 1, 0, 1);
  bitmatrix_set_bit(bms + 0, 1, 1, 1);
  bitmatrix_set_bit(bms + 0, 2, 1, 1);
  bitmatrix_set_bit(bms + 0, 2, 2, 1);
